...
void calc_crc::parse(QString original)
{
    // This fills parse_result with bytes extracted from a string like
    // "0x12 34 56", "0x1234", "0b1010 0001", etc.
    // It removes spaces and ignores empty pieces so we don’t get
    // fake bytes that cause DLC mismatches.
    ...
    // Trim and normalise
    QString s = original.trimmed();

    // ---------- HEX with 0x prefix ----------
    if (s.startsWith("0x", Qt::CaseInsensitive)) {
        // Remove "0x" and all whitespace
        QString rem = s.mid(2);
        rem.remove(' ');
        rem.remove('\t');
        rem.remove('\n');
        ...
        for (int i = 0; i < rem.length(); i += 2) {
            ...
            bool ok = false;
            uint value = token.toUInt(&ok, 16);
            if (!ok)
                continue;   // skip invalid piece instead of adding garbage
            parse_result.append(static_cast<quint8>(value & 0xFF));
        }
        return;
    }
    ...
    if (s.startsWith("0b", Qt::CaseInsensitive)) {
        ...
        // Pad on the left so length is a multiple of 8 bits
        int mod = rem.length() % 8;
        if (mod != 0) {
            rem.prepend(QString(8 - mod, QLatin1Char('0')));
        }

        for (int i = 0; i < rem.length(); i += 8) {
            QString chunk = rem.mid(i, 8);
            quint8 value = 0;
            bool ok = true;
            for (int j = 0; j < 8; ++j) {
                QChar c = chunk[j];
                ...
                value = static_cast<quint8>((value << 1) | (c == '1' ? 1 : 0));
            }
            if (!ok)
                continue;
    ...
    // ---------- Fallback: treat as plain hex without 0x ----------
    // e.g. "12 34 56"
    QString rem = s;
    rem.remove(' ');
    rem.remove('\t');
...
// NEW: pack bits *without* stuffing (for Classical CAN CRC)
void calc_crc::can_packNoStuff(const std::vector<bool>& bits,
                               std::vector<uint8_t>& outBytes,
                               int& outBitCount)
{
    outBytes.clear();
    int bitPos      = 7;   // MSB position of the current byte (matches can_packBit)
    outBitCount     = 0;

    for (bool b : bits) {
        // Re-use the existing packing helper (MSB -> LSB)
        can_packBit(outBytes, bitPos, b);
        ++outBitCount;   // count how many frame bits we packed
    }
}
...
QString calc_crc::Can_CRC(QList<QString> data, QList<QString>* results, bool extendedId, bool remoteFrame){
    ...
    // ---- 3) Pack *destuffed* bits from SOF to end of DATA ----
    // Bosch CAN 2.0B: CRC is calculated over the destuffed bit stream
    // from Start of Frame (SOF) until the last bit of the DATA field.
    std::vector<uint8_t> frameBytes;
    int frameBitCount = 0;
    can_packNoStuff(bits, frameBytes, frameBitCount);

    // ---- 4) CRC-15(CAN) over destuffed bitstream ----
    // Polynomial 0x4599, init 0x0000, no reflection, no final XOR.
    uint16_t crc15 = CalculateBits<uint16_t, 15>(frameBytes.data(),
                                                 static_cast<size_t>(frameBitCount),
                                                 CAN15table);
    crc15 &= 0x7FFF;   // keep 15 bits
