...
void crc_main::invokeCanCRC(bool fd, bool xl){
    ...
    if(fd == 0 && xl == 0){
        ...
        if(fd == 0 && xl == 0){
            ...
            // Gather inputs (don’t hard-gate on inputOkList; calculator validates)
            ...
            dataToParse.append(stringRemoveDelimiters(ui->lineEditCAN_Identifier->text())); // 11 or 29 bits
            dataToParse.append(stringRemoveDelimiters(ui->lineEditCAN_DLC->text()));        // 4 bits

            if(remoteFrame){
                // Remote frame: no payload; give an empty string
                dataToParse.append("0x"); // empty payload
            } else {
                dataToParse.append(stringRemoveDelimiters(ui->lineEditCAN_Payload->text())); // 0..8 bytes
            }

            QList<QString> results;
            QString err = Calculator->Can_CRC(dataToParse, &results, extendedId, remoteFrame);

            if(err != "success"){
                if(err == "failure dlcMismatch"){
                    showErrorBox("Fehler: DLC passt nicht zur Länge der Nutzdaten (Classical CAN).");
                } else {
                    showErrorBox("Fehler in CAN-Berechnung: " + err);
                }
                return;
            }
...
// =======================================
// Ethernet tab: "CRC Summe berechnen" button
// Requires these widgets (objectNames) in the Ethernet tab:
//   - QPlainTextEdit  plainEthPayload  : input ("0x..."/"0b...")
//   - QLineEdit       lineEthOutHex    : output hex (readOnly)
//   - QLineEdit       lineEthOutBin    : output bin (readOnly)
//   - QPushButton     btnEthCalc       : this button (auto-connected)
// =======================================
void crc_main::on_btnEthCalc_clicked()
{
    // Gather input
    QList<QString> data;
    data.append(ui->plainEthPayload->text().trimmed());

    // Call calculator
    QList<QString> results;
    QString status = Calculator->Eth_CRC(data, &results);

    if (status != "success") {
        QMessageBox::warning(this, "Ethernet CRC", status);
        return;
    }

    // Push results to UI fields and also to the ResultWindow (optional)
    QString hex, bin;
    for (int i = 0; i + 1 < results.size(); i += 2) {
        const QString &key = results[i];
        const QString &val = results[i + 1];
        if (key == "Ethernet CRC-32 (hex)") hex = val;
        if (key == "Ethernet CRC-32 (bin)") bin = val;
    }

    if (ui->lineEthOutHex) ui->lineEthOutHex->setText(hex);
    if (ui->lineEthOutBin) ui->lineEthOutBin->setText(bin);

    // Optional: show in the common ResultWindow, same style as other tabs
    // (Uncomment if you want the popup table)
    /*
    ResultWindow rw;
    rw.showResult(results);
    rw.exec();
    */
}
...
// Switch identifier rule between 11 and 29 bits when "Extended ID" toggles
// --- CAN: Extended ID toggle updates label + validation ---
void crc_main::can_updateIdentifierRules(bool checked)
{
    // If your label has a different objectName, change it here:
    if (ui->labelCAN_Identifier)
        ui->labelCAN_Identifier->setText(checked ? "Identifier (29-Bit)"
                                                 : "Identifier (11-Bit)");
    preliminary_parse();
}

// --- CAN: RTR toggle disables payload and clears it ---
void crc_main::can_updateRtrUi(bool checked)
{
    ui->lineEditCAN_Payload->setEnabled(!checked);
    if (checked) {
        ui->lineEditCAN_Payload->clear();
        UpdateStatusLabel("lineEditCAN_Payload", 0, "(OK) 0 Bits.");
    }
    preliminary_parse();
}
